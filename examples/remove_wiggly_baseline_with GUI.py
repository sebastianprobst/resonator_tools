...
from _example_setup import ensure_repo_root_on_path
...
S11 = noise * port1._S11_directrefl(
    freq, fr=fr, Ql=Qi * Qc / (Qc + Qi), Qc=Qc, a=1.0, alpha=0.0, delay=0.0
)
# add wiggly baseline
baseline = (
    1.0
    + 0.1 * np.cos(2.0 * np.pi * 0.001e-4 * freq)
    + 0.05 * np.sin(2.0 * np.pi * 0.0069e-4 * freq)
    + 0.001 * np.sin(2.0 * np.pi * 0.0001e-4 * freq)
)
S11b = S11 * baseline
# -----------------------------------------
